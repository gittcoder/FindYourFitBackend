...
def index():
    ...
    print(list_obj)
    color=str(dominant_color[0])+"$"+str(dominant_color[1])+"$"+str(dominant_color[2])
    t=[list_obj[0]["label"],color]
    cv2.imwrite("./images/test6_clothes_detected.jpg", cv2.cvtColor(img_with_boxes, cv2.COLOR_RGB2BGR)*255)
    cv2.imshow("Categorized Cloth",img_with_boxes)
    cv2.waitKey(0)
